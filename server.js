 ...
 */
app.post("/url", [ body("url").isURL().trim().escape() ], jsonParser, function (req, res, next) {
...
app.get("/url/:hash",
  [
    param("hash").isBase64().trim()
  ]
  , urlencodedParser, function (req, res) {
  console.log("reaching GET /url/:hash ", req.params.hash);
  var baseid = req.params.hash;
  var id = atob(baseid);
  URL.findOne({ _id: id }, function (err, doc) {
    console.log(id);
    if (doc) {
      console.log('URL found: ', doc.url);
      res.redirect(doc.url);
    } else {
      console.log('No url found. Redirecting to root. ', baseid);
      res.redirect("/");
    }
  });
});
...
app.get("/saved", jsonParser, function (req, res) {
  console.log("REACHING GET /saved");
  let saved = [];
